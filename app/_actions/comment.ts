...
export async function patchCommentAction(commentId: string, commentData: AddCommentType) {
    try {
        ...
        let message = "Unexpected error editing comment";

        if (axios.isAxiosError(error)) {
            const data = error.response?.data as APIError | undefined;
            message = data?.message || data?.error || "Failed to edit comment";
        }
...
export async function getReplyAction(postId: string, commentId: string) {
    try {
        const res = await api.get(`/comments/${postId}?commentId=${commentId}`);
        return { success: true, data: res.data };
    } catch (error) {
